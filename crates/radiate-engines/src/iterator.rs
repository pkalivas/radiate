...
use crate::{Generation, Limit, control::EngineControl, generation::GenerationRef, init_logging};
    ...
    E: Engine,
{
    ...
    limits: Vec<Box<dyn Fn(&E::Epoch) -> bool>>,
}
...
impl<C, T, E> EngineRunner<E>
where
    C: Chromosome,
    T: Clone,
    E: Engine<Epoch = Generation<C, T>>,
{
    pub fn until_score(mut self, score: impl Into<Score>) -> Self {
        let target_score = score.into();
        self.limits
            .push(Box::new(move |epoch: &E::Epoch| match epoch.objective() {
                Objective::Single(obj) => match obj {
                    Optimize::Minimize => epoch.score() > &target_score,
                    Optimize::Maximize => epoch.score() < &target_score,
                },
                Objective::Multi(objs) => {
                    let mut all_pass = true;
                    for (i, score) in epoch.score().iter().enumerate() {
                        let passed = match objs[i] {
                            Optimize::Minimize => score > &target_score[i],
                            Optimize::Maximize => score < &target_score[i],
                        };
    ...
    pub fn until_seconds(mut self, seconds: f64) -> Self {
        let stop_cond = Duration::from_secs_f64(seconds);
        self.limits
            .push(Box::new(move |epoch: &E::Epoch| epoch.time() >= stop_cond));
