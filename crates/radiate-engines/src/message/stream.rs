use crate::{
    Actor,
    message::actor::{Addr, Handler, Message},
};
...
impl<T: Send + Sync + Debug + 'static> Event for T {}
...
pub struct EventCtx(EventId, EventStream, tracing::Span);
...
impl EventCtx {
    ...
    pub fn publish<M: Event>(&self, message: M) {
        self.1.publish_within(message, &self.2);
    }
...
struct Mailbox {
    id: MailboxId,
    sender: channel::Sender<QueuedMessage>,
    receiver: channel::Receiver<QueuedMessage>,
    handler: Callback,
        ...
        loop {
            {
                ...
                while let Some(msg) = self.receiver.try_recv().ok() {
                    ...
                    let msg_ctx = EventCtx(msg.id, ctx.1.clone(), msg.span.clone());
                    (*handler)(msg.payload.as_ref(), &msg_ctx);
                    processed += 1;
                }
            }
            ...
            (*handler)(
                msg.payload.as_ref(),
                &EventCtx(msg.id, ctx.1.clone(), msg.span.clone()),
            );
            processed += 1;
        }
...
impl EventStream {
    ...
    pub fn publish<M: Event>(&self, message: M) {
        let id = EventId::new();
        let ty = M::event_label();
        let span = tracing::info_span!("event", ty = %ty, %id);
        self.dispatch(message, id, span);
    }

    fn publish_within<M: Event>(&self, message: M, parent: &tracing::Span) {
        let id = EventId::new();
        let ty = M::event_label();
        let span = tracing::info_span!(parent: parent, "event", ty = %ty, %id);
        self.dispatch(message, id, span);
    }

    fn dispatch<M: Event>(&self, message: M, id: EventId, span: tracing::Span) {
        let _enter = span.enter();
        let _publish_span = tracing::info_span!("publishing");
        ...
        let ctx = Arc::new(EventCtx(id, self.clone(), span.clone()));
