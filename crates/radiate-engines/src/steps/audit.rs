...
use radiate_core::{
    BatchMetricUpdater, Chromosome, Ecosystem, Metric, MetricSet, metric, metric_names,
...
impl AuditStep {
    ...
    fn calc_species_metrics<C: Chromosome>(
        ...
        if let Some(species) = ecosystem.species() {
            ...
            metrics.update(vec![new_species_count, species_ages, species_count]);
        } else {
    ...
    fn calc_membership_metrics<C: Chromosome>(
        ...
        metrics.update(vec![
            ...
            metric!(metric_names::SURVIVOR_COUNT, survivor_count),
