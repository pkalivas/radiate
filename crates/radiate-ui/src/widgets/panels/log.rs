...
impl<C: Chromosome> AppWidget<C> for ImprovementLogWidget {
    fn render(&self, area: Rect, buf: &mut Buffer, state: &mut AppState<C>) {
        if state.evo.pareto.objective.is_single() {
            render_improvement_log(area, buf, state);
        } else {
            render_front_event_log(area, buf, state);
        }
    }
}

fn render_improvement_log<C: Chromosome>(area: Rect, buf: &mut Buffer, state: &mut AppState<C>) {
    ...
    let rows = log
        ...
        .map(|entry| {
            ...
            Row::new(vec![
                ...
                Cell::from(format!("{:.6}", entry.score)).style(Style::default().fg(score_color)),
...
fn render_front_event_log<C: Chromosome>(area: Rect, buf: &mut Buffer, state: &mut AppState<C>) {
            ...
            Row::new(vec![
                ...
                Cell::from(Line::from(vec![
                    ...
                    Span::styled(
                        ...
                        Style::default().fg(crate::styles::sentiment_color(add_ratio, 0.2, 0.6)),
                    ),
                ])),
                Cell::from(Line::from(vec![
                ...
                Cell::from(Line::from(vec![
                    ...
                    Span::styled(
                        ...
                        Style::default().fg(crate::styles::sentiment_color(comp_ratio, 0.2, 0.6)),
                    ),
                ])),
