...
fn main() {
    ...
    let index = Expr::metric(metric_names::INDEX);
    ...
    let decay = Expr::lit(0.5_f32).pow(Expr::metric(metric_names::INDEX).div(half_life));
    ...
    let stagnant = Expr::metric("scores.best").stagnation(1e-4_f32).gte(20_u32);
    ...
    let volatility_driven = Expr::metric("score.volatility")
        .rolling(20)
        .mean()
