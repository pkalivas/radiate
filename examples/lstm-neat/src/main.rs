...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let neat_env = NeatEnvironment::new()
        ...
        .set_reactivate(0.2)
        .set_activation_functions(vec![Activation::Sigmoid, Activation::Relu]);

    let num_evolve = 500;

    let data = MemoryTest::new();
    let starting_net = Neat::default()
        .input_size(1)
...
impl MemoryTest {
    ...
    pub fn show(&self, model: &mut Neat) {
        ...
        println!(
            "Input: {:?}, Expecting: {:?}, Guess: {:.2}",
            vec![1.0],
            vec![0.0],
            model.forward(&vec![1.0]).unwrap()[0]
        );
        println!(
            "Input: {:?}, Expecting: {:?}, Guess: {:.2}",
            vec![0.0],
            vec![0.0],
            model.forward(&vec![0.0]).unwrap()[0]
        );
        println!(
            "Input: {:?}, Expecting: {:?}, Guess: {:.2}",
            vec![0.0],
