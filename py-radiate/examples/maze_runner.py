...
#     def __init__(self, waypoints: List[MazeWaypoint], start_point: Tuple[float, float] = (0, 0)):
...
#         ax.scatter(self.start_x, self.start_y, c='green', s=150, marker='s', label='Start')
        
#         path_x = [self.start_x] + [wp.x for wp in permutation]
#         path_y = [self.start_y] + [wp.y for wp in permutation]

#         ax.plot(path_x, path_y, 'r-', linewidth=2, alpha=0.8, label=f'Path (Length: {path_length:.2f})')
...
#             ax.annotate(f'→{i+1}', (wp.x, wp.y), xytext=(10, 10), textcoords='offset points', 
...
# def create_sample_maze() -> MazeSolver:
#     """Create a sample maze with waypoints."""
#     waypoints = [
#         MazeWaypoint(2, 3, "A"),
#         MazeWaypoint(5, 1, "B"),
#         MazeWaypoint(8, 4, "C"),
...
#     return MazeSolver(waypoints, start_point=(0, 0))


# def run_maze_evolution(maze_solver: MazeSolver, generations: int = 100) -> rd.Generation:
...
#     print(f"  Path: Start → {' → '.join([wp.name for wp in best_permutation])} → End")
    
#     # Visualize the solution
#     maze_solver.visualize_path(
#         best_permutation,
#         title=f"Best Maze Path (Length: {best_fitness})"
#     )
    
#     # Show evolution progress
#     print("\nEvolution Statistics:")
#     print(f"  Generations completed: {result.index()}")
#     print(f"  Best fitness: {result.score()}")
