...
class Cross(AlterBase):
    ...
    @staticmethod
    def sbx(rate: OperatorRate = 0.1, contiguity: float = 0.5) -> Cross:
        """
        ...
        """
        return Cross(
            components.SIMULATED_BINARY_CROSSOVER,
            rate=_get_rate(rate),
    ...
    @staticmethod
    def pmx(rate: OperatorRate = 0.1) -> Cross:
        """
        ...
        """
        return Cross(
            components.PARTIALLY_MAPPED_CROSSOVER,
            rate=_get_rate(rate),
            allowed_genes=GeneType.PERMUTATION,
        )

    @staticmethod
    def multipoint(rate: OperatorRate = 0.1, num_points: int = 2) -> Cross:
        """
        ...
        """
        return Cross(
            components.MULTI_POINT_CROSSOVER,
            rate=_get_rate(rate),
            num_points=num_points,
        )

    @staticmethod
    def mean(rate: OperatorRate = 0.1) -> Cross:
        """
        ...
        """
        return Cross(
            components.MEAN_CROSSOVER,
            rate=_get_rate(rate),
            allowed_genes={GeneType.FLOAT, GeneType.INT},
        )

    @staticmethod
    def uniform(rate: OperatorRate = 0.1) -> Cross:
        """
        ...
        """
        return Cross(
            components.UNIFORM_CROSSOVER,
            rate=_get_rate(rate),
            allowed_genes=GeneType.core(),
        )

    @staticmethod
    def blend(rate: OperatorRate = 0.1, alpha: float = 0.5) -> Cross:
        """
    ...
    @staticmethod
    def intermediate(rate: OperatorRate = 0.1, alpha: float = 0.5) -> Cross:
        """
        ...
        """
        return Cross(
            components.INTERMEDIATE_CROSSOVER,
            rate=_get_rate(rate),
            alpha=alpha,
            allowed_genes=GeneType.FLOAT,
        )
    ...
    def shuffle(rate: OperatorRate = 0.1) -> Cross:
        ...
        """
        return Cross(
            components.SHUFFLE_CROSSOVER,
            rate=_get_rate(rate),
            allowed_genes=GeneType.core(),
        )

    @staticmethod
    def edge_recombination(rate: OperatorRate = 0.1) -> Cross:
        """
        ...
        **Example**: If Parent 1 has edges (A-B, B-C) and Parent 2 has edges (B-C, C-D), the offspring might inherit edges (A-B, C-D).
        ...
        """
        return Cross(
            components.EDGE_RECOMBINE_CROSSOVER,
            rate=_get_rate(rate),
            allowed_genes=GeneType.PERMUTATION,
        )

    @staticmethod
    def graph(rate: OperatorRate = 0.5, parent_node_rate: OperatorRate = 0.5) -> Cross:
        """
        ...
        """
        return Cross(
            components.GRAPH_CROSSOVER,
            rate=_get_rate(rate),
            parent_node_rate=_get_rate(parent_node_rate),
            allowed_genes=GeneType.GRAPH,
        )

    @staticmethod
    def tree(rate: OperatorRate = 0.1, max_size: int = 30) -> Cross:
        """
...
class Mutate(AlterBase):
    ...
    @staticmethod
    def uniform(rate: OperatorRate = 0.1) -> Mutate:
        """
    ...
    def op(rate: OperatorRate = 0.1, replace_rate: OperatorRate = 0.1) -> Mutate:
        ...
        """
        return Mutate(
            components.OPERATION_MUTATOR,
            rate=_get_rate(rate),
            replace_rate=_get_rate(replace_rate),
    ...
    def graph(
        ...
        """
        return Mutate(
            components.GRAPH_MUTATOR,
            vertex_rate=_get_rate(vertex_rate),
            edge_rate=_get_rate(edge_rate),
    ...
    @staticmethod
    def scramble(rate: OperatorRate = 0.1) -> Mutate:
        """
        ...
        """
        return Mutate(
            components.SCRAMBLE_MUTATOR,
            rate=_get_rate(rate),
            allowed_genes=GeneType.core(),
        )

    @staticmethod
    def swap(rate: OperatorRate = 0.1) -> Mutate:
        """
        ...
        """
        return Mutate(
            components.SWAP_MUTATOR,
            rate=_get_rate(rate),
            allowed_genes=GeneType.core(),
        )

    @staticmethod
    def hoist(rate: OperatorRate = 0.1) -> Mutate:
        """
        ...
        """
        return Mutate(
            components.HOIST_MUTATOR,
            rate=_get_rate(rate),
            allowed_genes=GeneType.TREE,
        )

    @staticmethod
    def inversion(rate: OperatorRate = 0.1) -> Mutate:
        """
        ...
        """
        return Mutate(
            components.INVERSION_MUTATOR,
            rate=_get_rate(rate),
            allowed_genes=GeneType.core(),
        )

    @staticmethod
    def polynomial(rate: OperatorRate = 0.1, eta: float = 20) -> Mutate:
        """
    ...
    @staticmethod
    def jitter(rate: OperatorRate = 0.1, magnitude: float = 0.01) -> Mutate:
        """
    ...
    @staticmethod
    def arithmetic(rate: OperatorRate = 0.1) -> Mutate:
        """
...
#     def __init__(self, rate: OperatorRate = 0.1, alpha: float = 0.5):
#         """
#         The `BlendCrossover` is a crossover operator designed for `ArithmeticGene`s (IntGene & FloatGene).
#         It introduces variability by blending the `gene` controlled by the `alpha` parameter.
#         This approach allows for smooth transitions between `gene` values, promoting exploration of the search space.
...
#     def __init__(self, rate: OperatorRate = 0.5):
#         """
#         The `MeanCrossover` operator is a crossover mechanism designed
#         for `ArithmeticGene`s. It combines the corresponding `genes` of two parent chromosomes by
#         replacing a gene in the first parent with the mean (average) of the two `genes`. This approach
...
#     def __init__(self, rate: OperatorRate = 0.1):
#         """
#         The `ShuffleCrossover` is a crossover operator used in genetic algorithms,
#         particularly when working with permutations or chromosomes where order matters.
#         It works by shuffling the order in which genes are exchanged between two parent chromosomes
...
#     def __init__(self, rate: OperatorRate = 0.1, contiguity: float = 0.5):
#         """
#         The `SimulatedBinaryCrossover` is a crossover operator designed for `FloatGene`s.
#         It simulates binary crossover by creating offspring that are a linear combination of the parents, controlled
#         by a contiguity factor. Effectively, it allows for a smooth transition between parent values
...
#     def __init__(self, rate: OperatorRate = 0.1, num_points: int = 2):
#         """
#         The `MultiPointCrossover` is a crossover operator that combines two parent individuals by selecting
#         multiple crossover points and swapping the genetic material between the parents at those points. This is a
#         classic crossover operator.
...
#             args={
#                 "vertex_rate": vertex_rate.__backend__()
#                 if isinstance(vertex_rate, Expr)
#                 else Expr.lit(vertex_rate),
#                 "edge_rate": edge_rate.__backend__()
...
#     def __init__(self, rate: OperatorRate = 0.5, max_size: int = 30):
#         """
#         The `TreeCrossover` is a crossover operator that randomly selects a subtree from one parent tree and
#         swaps it with a subtree from another parent tree.
...
#     def __init__(self, rate: OperatorRate = 0.5, eta: float = 20.0):
#         """
#         The `PolynomialMutator` applies a polynomial mutation to the genes of a chromosome.
#         This provides a bounded and unbiased mutation to genes where you care about the distribution of the mutation.
#         Unlike Gaussian mutation, Polynomial can give more control over the tail behavior.
...
#     def __init__(self, rate: OperatorRate = 0.1, magnitude: float = 0.1):
#         """
#         The `JitterMutator` adds small random perturbations to the values of a `gene`
#         within a `chromosome`. A random value is sampled from a uniform
#         distribution between [-1, 1], then it is scaled by the `magnitude`
