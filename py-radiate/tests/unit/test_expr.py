...
def test_when_then_expr_with_literal_then(simple_metric_set):
    expr = (
        rd.when(rd.metric("one").max().cast(rd.Float64) == 9.0).then(111).otherwise(222)
    )
...
def test_when_then_expr_with_literal_otherwise(simple_metric_set):
    expr = rd.when(rd.metric("one").max() < 0).then(111).otherwise(222)
    assert simple_metric_set.project(expr) == 222


@pytest.mark.unit
def test_nested_when_then(simple_metric_set):
    inner = rd.when(rd.metric("two").mean() > 14.0).then(1).otherwise(2)
    outer = (
        rd.when(rd.metric("one").max().cast(rd.Float64) == 9.0).then(inner).otherwise(3)
    )
