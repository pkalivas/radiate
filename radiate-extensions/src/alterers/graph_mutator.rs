...
impl NodeMutate {
    pub fn rate(&self) -> f32 {
        match self {
            NodeMutate::Forward(rate) => *rate,
            NodeMutate::Recurrent(rate) => *rate,
        }
    }

    pub fn is_recurrent(&self) -> bool {
        match self {
            NodeMutate::Forward(_) => false,
            NodeMutate::Recurrent(_) => true,
        }
    }
}
...
where
    T: Clone,
{
    pub fn new(mutations: Vec<NodeMutate>, operations: Vec<Expr<T>>) -> Self {
        Self {
            mutations,
            operations,
        }
    ...
    #[inline]
    fn add_node(&self, graph: &mut Graph<T>) {
        let source_node = random_source_node(graph.nodes()).index;
        let target_node = random_target_node(graph.nodes()).index;

        if !can_connect(graph.nodes(), source_node, target_node, false) {
            return;
        }

        let expr = random_provider::choose(&self.operations);

        let new_idx = graph.add_node(expr.new_instance());

        graph.attach(source_node, new_idx);
        graph.attach(new_idx, target_node);
        graph.detach(source_node, target_node);

        self.repair_insert(graph, new_idx, false);
    }
    ...
    fn repair_insert(
        ...
        let arity = *collection.get(new_node_index).cell.value.arity();
        if arity == 0 {
            return true;
        }
        for _ in 0..arity - 1 {
            let other_source_node = random_source_node(collection.nodes());
            if can_connect(
                collection.nodes(),
        ...
        if !collection.nodes().iter().all(|node| node.is_valid()) {
            return false;
        }
...
pub fn random_source_node<T>(collection: &[GraphNode<T>]) -> &GraphNode<T> {
    ...
    while seen.len() < collection.len() {
        let index = random_provider::random::<usize>() % collection.len();
        if !seen.contains(&index) {
            seen.insert(index);
            let node = collection.get(index).unwrap();
            if node.cell.role.allows_outgoing() {
                return node;
            }
        }
    }

    panic!("No source node found in collection");
}
...
pub fn random_target_node<T>(collection: &[GraphNode<T>]) -> &GraphNode<T> {
    ...
    while seen.len() < collection.len() {
        ...
        if !seen.contains(&index) {
            seen.insert(index);
            let node = collection.get(index).unwrap();
            if node.cell.role.allows_incoming() {
                return node;
            }
        }
    }

    panic!("No target node found in collection");
}
...
impl<T> Alter<NodeChrom<GraphNode<T>>> for GraphMutator<T>
where
    T: Clone + PartialEq + Default + 'static,
{
    ...
    fn alter(
        ...
        for i in 0..population.len() {
            ...
            let cloneed_graph = chromosome
                .get_genes()
                .iter()
                .cloned()
                .collect::<Vec<GraphNode<T>>>();
            let mut graph = Graph::new(cloneed_graph);

            self.add_node(&mut graph);

            if !graph.is_valid() {
                continue;
            }

            if graph.len() == chromosome.get_genes().len() {
                continue;
            }
            ...
            copied_genotype[chromosome_index] = NodeChrom::new(graph.nodes().to_owned());
            ...
            //     } else {
            //         let node_fact = factory.borrow();
            //         let cloneed_graph = chromosome.nodes.clone();
            //         let graph = Graph::new(cloneed_graph);
            //         self.add_node(&chromosome.nodes, &mutation.node_type(), &node_fact)
            //     };
