...
pub struct GraphArchitect<C: NodeCell> {
    store: CellStore<C>,
}

impl<C: NodeCell> GraphArchitect<C> {
    pub fn new(store: CellStore<C>) -> Self {
        GraphArchitect { store }
    }
}
...
impl<C: NodeCell + Clone + Default> GraphArchitect<C> {
...
impl<C: NodeCell> Default for GraphArchitect<C> {
    fn default() -> Self {
        GraphArchitect {
