...
// /// This implementation is for the `GraphChromosome` type.
// impl<C, G, T> Alter<C> for OperationMutator
// where
//     C: Chromosome<Gene = G>,
//     G: Gene<Allele = Op<T>>,
//     T: Clone + PartialEq + Default,
...
// /// If the node is not a `MutableConst` node, it tries to replace it with a new node from the store,
// /// but only if the arity of the new node is the same as the current node.
// impl<C, G, T> Mutate<C> for OperationMutator
// where
//     C: Chromosome<Gene = G>,
//     G: Gene<Allele = Op<T>>,
//     T: Clone + PartialEq + Default,
...
//                     let swap_gene = chromosome.get_gene(rand_idx % chromosome.len());

//                     if swap_gene.allele().arity() == current_node.allele().arity() {
//                         let swap_allele = swap_gene.allele();
//                         chromosome.set_gene(i, current_node.with_allele(swap_allele));
//                     }
//                 }
//             }
//         }
