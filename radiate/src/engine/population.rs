    ...
    P: Problem<T>,
{
        ...
        P: Send + Sync,
    {
        ...
        self.curr_gen
            .speciate(self.config.distance, Arc::clone(&self.environment));
        ...
        self.curr_gen = self.curr_gen.create_next_generation(
            self.size,
            self.config.clone(),
    ...
    fn manage_stagnation(&mut self, curr_top_score: f32) {
        ...
        } else if (curr_top_score - self.stagnation.previous_top_score).abs() < f32::EPSILON {
