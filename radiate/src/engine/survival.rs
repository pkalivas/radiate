...
impl SurvivalCriteria {
    ...
    #[inline]
    pub fn pick_survivors<T, E>(
        &self,
        members: &mut [Container<T, E>],
        families: &[Family<T, E>],
        ...
        E: Send + Sync,
    {
        match self {
            ...
            Self::TopNumber(num) => SurvivalCriteria::get_top_num(*num, members),
    ...
    where
        ...
        E: Send + Sync,
    {
        members.par_sort_by(|a, b| b.fitness_score.partial_cmp(&a.fitness_score).unwrap());
        Some(
            (0..num_to_keep)
                .into_iter()
...
impl ParentalCriteria {
    ...
    pub fn pick_parents<T, E>(
        ...
        inbreed_rate: f32,
        families: &[Family<T, E>],
    ) -> Option<((f32, Member<T>), (f32, Member<T>))>
    where
        T: Genome<T, E> + Send + Sync + Clone,
        E: Send + Sync,
    {
        match self {
            Self::BiasedRandom => Some(self.create_match(inbreed_rate, families)),
            Self::BestInSpecies => {
                let mut r = rand::thread_rng();
                let child_one = families.choose(&mut r)?.read().unwrap().fittest();
                let child_two = families.choose(&mut r)?.read().unwrap().fittest();
                Some((child_one, child_two))
            }
        }
    }
    ...
    #[inline]
    fn get_biased_random_species<T, E>(
        &self,
        r: &mut ThreadRng,
        families: &[Family<T, E>],
    ) -> Option<Family<T, E>>
        ...
        E: Send + Sync,
    {
        ...
        let mut result = None;
        let total = families.iter().fold(0.0, |sum, curr| {
