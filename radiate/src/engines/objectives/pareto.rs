...
pub fn crowding_distance(scores: &Vec<&[f32]>, objective: &Objective) -> Vec<f32> {
    let indices = scores
        .iter()
        .enumerate()
        .map(|(i, score)| (*score, i))
...
pub fn rank(population: &Vec<&[f32]>, objective: &Objective) -> Vec<usize> {
...
pub fn weights(scores: &Vec<&[f32]>, objective: &Objective) -> Vec<f32> {
