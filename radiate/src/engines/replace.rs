use super::{
    Chromosome, EngineError, Genotype, Phenotype, Population, ProblemError, random_provider,
};
use std::sync::Arc;
...
pub trait ReplacementStrategy<C: Chromosome> {
    fn replace(
        ...
        encoder: Arc<dyn Fn() -> Result<Genotype<C>, ProblemError>>,
    ) -> Result<(), ProblemError>;
}
...
impl<C: Chromosome> ReplacementStrategy<C> for PopulationSampleReplace {
    fn replace(
        ...
        generation: usize,
        population: &mut Population<C>,
        _: Arc<dyn Fn() -> Result<Genotype<C>, ProblemError>>,
    ) -> Result<(), ProblemError> {
