...
impl LSTM {
    pub fn new(
        ...
        LSTM {
            ...
            g_gate: Arc::new(RwLock::new(Dense::new(
                cell_input,
                memory_size,
                LayerType::DensePool,
                Activation::Tanh,
            ))),
            i_gate: Arc::new(RwLock::new(Dense::new(
                cell_input,
                memory_size,
                LayerType::DensePool,
                Activation::Sigmoid,
            ))),
            f_gate: Arc::new(RwLock::new(Dense::new(
                cell_input,
                memory_size,
                LayerType::DensePool,
                Activation::Sigmoid,
            ))),
            o_gate: Arc::new(RwLock::new(Dense::new(
                cell_input,
                memory_size,
                LayerType::DensePool,
                Activation::Sigmoid,
            ))),
            v_gate: Arc::new(RwLock::new(Dense::new(
                memory_size,
                output_size,
                LayerType::DensePool,
                activation,
    ...
    pub fn step_forward_async(&mut self, inputs: &[f32]) -> Option<Vec<f32>> {
        ...
        // spawn the threads
        let g_output = thread::spawn(move || {
            return g_gate_clone.write().unwrap().forward(&*g_input).unwrap();
        });
        let o_output = thread::spawn(move || {
            return o_gate_clone.write().unwrap().forward(&*o_input).unwrap();
        });
        let f_output = thread::spawn(move || {
            return f_gate_clone.write().unwrap().forward(&*f_input).unwrap();
        });
        let i_output = thread::spawn(move || {
            return i_gate_clone.write().unwrap().forward(&*i_input).unwrap();
        });
        ...
        vectorops::element_multiply(
            &mut curr_output,
            &vectorops::element_activate(&self.memory, Activation::Tanh),
        );

        // update the state parameters only if the gates are traceable and the data needs to be collected
        self.states
            .update_forward(f_curr, i_curr, g_out, o_out, self.memory.clone());
    ...
    pub fn step_forward(&mut self, inputs: &[f32]) -> Option<Vec<f32>> {
        ...
        // current memory and output need to be mutable but we also want to save that data for bptt
        let mut current_state = g_output;
        let mut current_output = o_output;
    ...
    #[inline]
    pub fn step_back(&mut self, errors: &[f32], l_rate: f32) -> Option<Vec<f32>> {
        ...
        vectorops::element_multiply(
            &mut dho,
            &vectorops::element_deactivate(&o_curr, self.o_gate.read().unwrap().activation),
        );
        let o_gate_clone = Arc::clone(&self.o_gate);
        ...
        vectorops::element_multiply(
            &mut dc,
            &vectorops::element_deactivate(&c_old, Activation::Tanh),
        );
        ...
        vectorops::element_multiply(
            &mut dhf,
            &vectorops::element_deactivate(&f_curr, self.f_gate.read().unwrap().activation),
        );
        let f_gate_clone = Arc::clone(&self.f_gate);
        ...
        vectorops::element_multiply(
            &mut dhi,
            &vectorops::element_deactivate(&i_curr, self.i_gate.read().unwrap().activation),
        );
        let i_gate_clone = Arc::clone(&self.i_gate);
        ...
        vectorops::element_multiply(
            &mut dhc,
            &vectorops::element_deactivate(&g_curr, self.g_gate.read().unwrap().activation),
        );
        let g_gate_clone = Arc::clone(&self.g_gate);
...
impl Layer for LSTM {
    ...
    #[inline]
    fn backward(&mut self, errors: &[f32], learning_rate: f32) -> Option<Vec<f32>> {
    ...
    LSTM: Layer,
{
    ...
    fn crossover(
        ...
        let child = LSTM {
            ...
            states: LSTMState::new(),
            g_gate: Arc::new(RwLock::new(Dense::crossover(
                &child.g_gate.read().unwrap(),
                &parent_two.g_gate.read().unwrap(),
                Arc::clone(&env),
                crossover_rate,
            )?)),
            i_gate: Arc::new(RwLock::new(Dense::crossover(
                &child.i_gate.read().unwrap(),
                &parent_two.i_gate.read().unwrap(),
                Arc::clone(&env),
                crossover_rate,
            )?)),
            f_gate: Arc::new(RwLock::new(Dense::crossover(
                &child.f_gate.read().unwrap(),
                &parent_two.f_gate.read().unwrap(),
                Arc::clone(&env),
                crossover_rate,
            )?)),
            o_gate: Arc::new(RwLock::new(Dense::crossover(
                &child.o_gate.read().unwrap(),
                &parent_two.o_gate.read().unwrap(),
                Arc::clone(&env),
                crossover_rate,
            )?)),
            v_gate: Arc::new(RwLock::new(Dense::crossover(
                &child.v_gate.read().unwrap(),
                &parent_two.v_gate.read().unwrap(),
                Arc::clone(&env),
                crossover_rate,
            )?)),
        };
    ...
    fn distance(one: &LSTM, two: &LSTM, env: Arc<RwLock<NeatEnvironment>>) -> f32 {
        let mut result = 0.0;
        result += Dense::distance(
            &one.g_gate.read().unwrap(),
            &two.g_gate.read().unwrap(),
            Arc::clone(&env),
        );
        result += Dense::distance(
            &one.i_gate.read().unwrap(),
            &two.i_gate.read().unwrap(),
            Arc::clone(&env),
        );
        result += Dense::distance(
            &one.f_gate.read().unwrap(),
            &two.f_gate.read().unwrap(),
            Arc::clone(&env),
        );
        result += Dense::distance(
            &one.o_gate.read().unwrap(),
            &two.o_gate.read().unwrap(),
            Arc::clone(&env),
        );
        result += Dense::distance(
            &one.v_gate.read().unwrap(),
            &two.v_gate.read().unwrap(),
            Arc::clone(&env),
        );
        result
    }
