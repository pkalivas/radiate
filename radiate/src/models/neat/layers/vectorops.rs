use super::super::{activation::Activation, loss::Loss};
...
pub fn element_multiply(one: &mut [f32], two: &[f32]) {
    assert!(
        one.len() == two.len(),
        "Element multiply vector shapes don't match"
    );
    one.iter_mut().zip(two.iter()).for_each(|(a, b)| *a *= b);
}

/// invert a vector that is already holding values between 0 and 1
#[inline]
pub fn element_invert(one: &mut [f32]) {
    one.iter_mut().for_each(|a| *a = 1.0 - *a);
}
...
pub fn element_add(one: &mut [f32], two: &[f32]) {
    assert!(
        one.len() == two.len(),
        "Element add vector shapes don't match"
    );
    one.iter_mut().zip(two.iter()).for_each(|(a, b)| *a += b);
}
...
pub fn element_activate(one: &[f32], func: Activation) -> Vec<f32> {
    one.iter().map(|x| func.activate(*x)).collect()
}

#[inline]
pub fn element_deactivate(one: &[f32], func: Activation) -> Vec<f32> {
    one.iter().map(|x| func.deactivate(*x)).collect()
}
...
pub fn softmax(one: &[f32]) -> Vec<f32> {
    let ex = one.iter().map(|x| x.exp()).collect::<Vec<_>>();
    ...
    match loss_fn {
        ...
        Loss::MSE => {
            ...
            let errs = one
                ...
                .collect::<Vec<_>>();
            ((1.0 / one.len() as f32) * squared_error, errs)
        }
    }
}
