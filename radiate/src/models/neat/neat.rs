...
use super::{
    ...
    layers::{dense::Dense, gru::GRU, layer::Layer, layertype::LayerType, lstm::LSTM, vectorops},
    loss::Loss,
...
impl Neat {
        ...
        F: Fn(usize, f32) -> bool,
    {
        ...
        assert!(
            ...
            "Input and target data are different sizes"
        );
        assert!(
            inputs[0].len() as u32 == self.input_size,
            "Input size is different than network input size"
        );
            ...
            for j in 0..inputs.len() {
                ...
                pass_out.push(
                    self.forward(&inputs[j])
                        .ok_or("Error in network feed forward")?,
                );
    ...
    pub fn backward(
        ...
        for i in (0..net_outs.len()).rev() {
            ...
            total_loss += errors.0;
            self.layers.iter_mut().rev().fold(errors.1, |res, curr| {
    ...
    #[inline]
    pub fn forward(&mut self, data: &[f32]) -> Option<Vec<f32>> {
    ...
    pub fn load(file_path: &str) -> Result<Neat, Box<dyn Error>> {
        Ok(
            serde_json::from_reader(File::open(file_path).expect("file not found"))
...
impl Genome<Neat, NeatEnvironment> for Neat {
    ...
    fn crossover(
        ...
        for (one_layer, two_layer) in one.layers.iter().zip(two.layers.iter()) {
            let new_layer: Box<dyn Layer> = match one_layer.layer_type {
                LayerType::Dense | LayerType::DensePool => Box::new(Dense::crossover(
                    one_layer.as_ref(),
                    two_layer.as_ref(),
                    Arc::clone(&env),
                    crossover_rate,
                )?),
                LayerType::LSTM => Box::new(LSTM::crossover(
                    one_layer.as_ref(),
                    two_layer.as_ref(),
                    Arc::clone(&env),
                    crossover_rate,
                )?),
                LayerType::GRU => Box::new(GRU::crossover(
                    one_layer.as_ref(),
                    two_layer.as_ref(),
                    Arc::clone(&env),
                    crossover_rate,
    ...
    fn base(env: &mut NeatEnvironment) -> Neat {
        Neat::default()
            .input_size(env.input_size.unwrap())
            .dense_pool(env.output_size.unwrap(), Activation::Sigmoid)
    }
