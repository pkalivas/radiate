...
use crate::thread_pool::{self, SingleFlag, ThreadPool, WaitGroup, WaitGuard};
    ...
    T: Clone + Send + 'static,
{
        ...
        Self: Sized,
    {
        ...
        Some(Box::new(FrontStep {
            update_guard: SingleFlag::new(), // Initialize the guard
    ...
    fn execute(&self, ctx: &mut EngineContext<C, T>) {
        // Check if we need to update the front
        // (*self.guard.lock().unwrap()) = self.wait_group.guard();
        ...
        let scope = self.thread_pool.scope();
        self.thread_pool.submit(move || {
            ...
            let dominates_vector = doms_vector
                ...
                // .map(|(idx, _)| Phenotype::clone(&new_individuals[idx]))
                .collect::<Vec<&Phenotype<C>>>();
        ...
        // for (idx, member) in new_individuals.iter().enumerate() {
        //     // We need to clone the member to avoid borrowing issues
        //     let pheno = Phenotype::clone(member);
        //     let front_clone = RwCell::clone(&front_clone);
        //     let doms_vector = RwCell::clone(&doms_vector);
        //     let remove_vector = RwCell::clone(&remove_vector);

        //     // Spawn a thread to check for dominance
        //     scope.spawn(move || {
        //         let (dominates, to_remove) = front_clone.read().dominates(&pheno);
        ...
        // let wg = self.thread_pool.scope(move |scope| {
        //     for (idx, member) in new_individuals.iter().enumerate() {
        //         let pheno = Phenotype::clone(member);
        //         let front_clone = RwCell::clone(&front_clone);
        //         let doms_vector = RwCell::clone(&doms_vector);
        //         let remove_vector = RwCell::clone(&remove_vector);
        //         println!("here: idx = {}", idx);
        //         scope.spawn(move || {
        //             let (dominates, to_remove) = front_clone.read().dominates(&pheno);
        ...
        // drop(scope); // This will wait for all threads to finish
